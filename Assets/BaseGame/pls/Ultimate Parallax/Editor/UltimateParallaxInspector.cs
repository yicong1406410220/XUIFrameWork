                    ...
                    if (!Application.isPlaying)
                    {
                        ...
                        if (curLayer.showSettings)
                        {
                            ...
                            if (curLayer.sprites.Count > 0)
                            {
                                ...
                                m = (Material)EditorGUILayout.ObjectField("Material", curSR.material, typeof(Material), false);
...
static class ReflectionUtility
{
    ...
    public static System.Type FindClass(string name) { return typeof(Editor).Assembly.GetType(name, false, true); }
    public static T InvokeMethod<T>(this System.Type type, string methodName, params object[] args) { return (T)type.FindMethod(methodName).Invoke(null, args); }
    public static MethodInfo FindMethod(this System.Type type, string methodName) { return type.GetMethod(methodName, FULL_BINDING); }
}